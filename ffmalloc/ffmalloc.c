...
static void my_mincore_test_all_ones(void *addr, size_t length, unsigned char *vec) {
...
static void my_mincore_test_one_then_all_zeros(void *addr, size_t length, unsigned char *vec) {
  if (length == 0) return;
  my_mincore(addr, length, vec);
  assert((vec[0] & 1) == 1);
  bool ok = true;
